...
11. [1x 30cm 4-wire white cable or 8-wire flat ribbon telephone cable for extension from motor to ceiling](https://www.amazon.de/dp/B07TGKPFHK/)
...
## Composition
Be careful, check that you have turned off the fuses of your power supply, otherwise you might severely hurt yourself. I know that even without being an electrician ;)
<p align="center">
...
As you can see in the graphic, the wiring is not complicated. The orange Wago clamps and the cable lugs provided an easy and comfortable way to set everything up. But now follows a little explanation:

The servo motor has four cables. The orange one (analog feedback) you plug into A0 of the Wemos. The white data cable you plug into D0. I don't supply the power for motor via the microcontroller, because it only gives up to 500mA via the pins, but the motor could overload this, by drawing up to 1800mA when it turns. The power for the motor and microcontroller comes from the power supply, which outputs 5V. To make the four cables fit to the 2 connectors, I feed + and - through each of the Wago clamps and put a jumper cable in there. Then I screwed the respective jumper cables (red: + and black: -) to the power supply by replacing the end with cable lugs.
I then connected the yellow ground cable from the ceiling to the ground of the power supply. Finally I attached cable lugs to two 10cm cables (with 1,5mm² thickness because of the higher current) to attach them at N and L between the power supply and the lamp as shown in the graphic with the brown and blue cable.

I later placed the servo motor in the lamp and put everything else up in the tray on the ceiling. Since the four cables from the motor are too short to reach from the ceiling to the center of the lamp, I used a white flat telephone cable and connected that as a 30cm extension to the cable ends from the motor and then the other end of the telephone cable into the microcontroller/Wago clamps. My telephone cable had 8 wires, where I then attached Dupont connectors myself, so that always 2 wires were together and I thus had 4. Alternatively you can use a 4-wire cable. (I got it as a drum of 100 meters of telephone cables, whose remains I still need to find some good use for. If you are from Germany near Berlin I can give you some :D)
...
That's all for the cabling. Now only the attachment of the sewing thread spool to the motor and then the attachment of the motor in the lamp is missing.
...
In line 5 and 6 of [IKEAPS2014.ino](https://github.com/julisa99/IKEAPS2014/blob/main/IKEAPS2014.ino) you have to enter your access data for your wifi. After that you can upload the code to your microcontroller. In the end you can ask Alexa to search for new devices and you should find the device "star".

## How it works
When you command "Alexa, open star to 20%"; the function `deviceChanged(brightness)` is executed, where the 20% is passed as `brightness` of "51", since brightness value is between 0-255.
Now I calculate how often the sewing thread spool has to rotate. I have set `MAX_ROTATION` to how many times my cord must be wound on the sewing thread spool until the lamp is fully open. In my case, `MAX_ROTATION` is 4500, which would be ~4.4 revolutions (=4500/1024). This is because the encoder of the motor provides values of 0-1024 for the rotation range of 0°-360°. So I calculate 20% of 4500 (=900) as target of the motor. 

Now I decide in which direction the motor has to rotate to achieve the target position. Because the encoder is sometimes a bit inaccurate, I use an offset for the target position, which can be adjusted arbitrarily. If the 20% is not reached, the lamp will either move open or close until the 20% is reached (depending on the last position; at 0% it would now opens to reach 20%).
